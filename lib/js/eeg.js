...

{
  ...
  layersOrder: [],                        // (optional) default to ['legend', 'links','linksLabelsBack','linksLabels', 'nodes']
  wrapForeignObjectHtmlInBodyTag: false,  // (optionsl) default false
...
(function (window, $, d3) {
  ...
  function Eeg(selector, opt) {
    ...
    var defaultOptions = {
      ...
      nodeR: 10,
      title: null,
      wrapForeignObjectHtmlInBodyTag: false,
      layersOrder: ['legend', 'links','linksLabelsBack','linksLabels', 'nodes'],
      showLegend: true,
      forceGravity: 0.01,
      forceDistance: 200,
    ...
    var _tryToDiscoverBaseURL = function () {
      var scriptSrc = $('script[src*="eeg.js"]').attr('src');
      var baseURL = '';
      if (scriptSrc) {
        var lastIndex = scriptSrc.lastIndexOf('/');
        baseURL = scriptSrc.substring(0, lastIndex + 1);
      } else {
        if (console && console.log) {
          console.log(
            'Could not figure out baseULR.\n' +
            'If you included the eeg.js script in the html you - it might be that you renamed it.\n' +
            'If you use tools like require - please make sure that the baseURL option is specified'
          );
        }
      }
      return baseURL;
    ...
    this.init = function (opt) {
      ...
      if (opt) {
        ...
        this.options.baseURL     = opt.baseURL !== undefined ? opt.baseURL : _tryToDiscoverBaseURL();
        ...
        this.options.wrapForeignObjectHtmlInBodyTag = opt.wrapForeignObjectHtmlInBodyTag || defaultOptions.wrapForeignObjectHtmlInBodyTag;
        this.options.layersOrder    = opt.layersOrder   || defaultOptions.layersOrder;
        this.options.showContextMenu = opt.showContextMenu || defaultOptions.showContextMenu;
    ...
    // create a container for eeg hidden stuff
    // needed to append elements to obtain dimentions
    function getRandomInt(min, max) {
      return Math.floor(Math.random() * (max - min + 1)) + min;
    }
    this.eegid = 'eeg-' + getRandomInt(0,  100000);
    $('body').append('<div id="' + this.eegid + '-hidden">');
    this.$hiddenElement = $('#' + this.eegid + '-hidden');
  ...
  Eeg.prototype = (function () {
    ...
    function _addSingleLink(link) {
      ...
      if (link.htmlElement) {
        ...
        var el = $('<div style="visibility: hidden; display: inline-block;">').append(htmlElement);
        this.$hiddenElement.append(el)
        // calculate the width and height
        htmlElementWidth = el.width() + 2;
        htmlElementHeight = el.height() + 2;
        this.$hiddenElement.empty();
      } else if (link.html) {
        var el = $('<div style="visibility: hidden; display: inline-block;">').html(link.html);
    ...
    var linkTextForeignSelection = linkTextForeign.enter()
    ...
    if (self.options.wrapForeignObjectHtmlInBodyTag) {
      linkTextForeignSelection = linkTextForeignSelection.append('xhtml:body');
    }

    linkTextForeignSelection.html(function (d) {
    ...
    .on('click', function (d, i) {      self.options.onLinkClick.call(self, this, d, i);  });
      ...
      var circle = nodeEnter.append('svg:circle')
        ...
        .on('contextmenu', function (d, i) {
          if (!self.options.showContextMenu) {
            return;
          };
    ...
    return {
      ...
      start: function () {
        ...
        // Add layers
        for (var i = 0; i < self.options.layersOrder.length; i++) {
          self.vis.append('g').attr('id', self.options.layersOrder[i] +'Layer');
        }
        ...
        /*
        Commenting as it seems not used anywhere
        ...
        */
        ...
        self.$el.append(contextMenuHtml);
