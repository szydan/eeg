...

{
  ...
  monitorContainerSize = true,// ( optional ) default false
...
function Eeg(selector, opt){
  ...
  // now set the default options
    ...
    this.options.minLinkSize = opt.minLinkSize || 0.8;
    ...
    this.options.alwaysShowLinksLabels = opt.alwaysShowLinksLabels || false;
    ...
    if( opt.onNodeUnsticky && !this._isFunction(opt.onNodeUnsticky) ){
...
Eeg.prototype = (function(){
  ...
  function _drawLegend(){
    ...
    function _addSingleLink(link){
        ...
        var htmlElement = link.htmlElement ? link.htmlElement : undefined;
        var htmlElementWidth = link.htmlElementWidth || 100;
        var htmlElementHeight = link.htmlElementHeight || 20;
  ...
  function _update(){
    ...
    }).data( filterLinks(true), function(d) { return d.source.id + "-" + d.target.id+"-"+d.no; });
    ...
    .html(function(d) {
      ...
      if (d.htmlElement) {
        return d.htmlElement.innerHTML;
      }
      return null;
    })
    .on("mouseover", function(d,i){  self.options.onLinkMouseOver.call(self, this,d,i);  })
    ...
    }).data(filterLinks(false), function(d) { return d.source.id + "-" + d.target.id+"-"+d.no; });
    ...
    .attr("width",  function(d){
        ...
        var linkTextEl = self.vis.select("#label-"+d.source.id + "-" + d.target.id+"-"+d.no)[0][0];
        if(linkTextEl){
    ...
    this.force.on("tick", function(e) {
        ...
        //TODO: here compute the background corectly for foreign object
        linkTextBack
        .attr("x", function(d) {
                var P0 = {
                    x: d.source.x,
        ...
        .attr("y", function(d) {
                ...
                if( bezierPoints.type === 'line'){
                    var dy = (d.source.y + d.target.y - d.linkTextBackBBox.height)/2;
                    if (!d.htmlElement) {
                ...
                // here compute middle point
                var M = _computePointOnCurve.call(self,bezierPoints.P0,bezierPoints.P1,bezierPoints.P2,bezierPoints.P3,0.5);
                var dy = M.y - d.linkTextBackBBox.height/2;
                if (!d.htmlElement) {
                  dy -= self.options.fontSize/2;
                }
                return  dy;
