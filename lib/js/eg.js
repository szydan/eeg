...
function Eg(selector, opt){
  ...
  } else if (selector.jquery && selector.attr('id')) {
    // in case user pass in already jQuery or JqLite element
    var idSelector = el.attr('id');
    if(idSelector) {
      this.selector = '#' + idSelector;
      this.$el = el; // use this one everywhere else
    } else {
      throw 'Error wrong Eg selector [selector]. Currenty only #sel is supported.'
    }
  } else {
    throw 'Error wrong Eg selector [selector]. Currenty only #sel is supported.'
  }
